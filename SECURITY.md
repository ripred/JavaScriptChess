...
- Platform, runtime, compiler, or dependency versions.
- Minimal command, input, or reproduction that demonstrates the issue.
- Whether the issue requires local access, network access, malicious input, or a hostile dependency.
- Expected impact and any known workaround.

## Scope

Security reports are most useful when they involve memory safety, unsafe command execution, unsafe generated code, build-time supply-chain risk, credential exposure, or behavior that lets untrusted input trigger unintended actions.
